...
namespace matplotlibcpp {
  ...
  inline void legend_upper_left()
  {
    PyObject* kw_args = PyDict_New();
    std::string pos = "upper left";
		PyDict_SetItemString(kw_args, "loc", PyString_FromString(pos.c_str()));
    PyObject* option_args = PyTuple_New(0);
    PyObject* res = PyObject_Call(detail::_interpreter::get().s_python_function_legend,option_args,kw_args);
    if(!res) throw std::runtime_error("Call to legend() failed.");
    Py_DECREF(res);
  }
